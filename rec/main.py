...
def resolve_and_download(rid: str, file_path: str, use_pc: bool):
    s = douyu.DouYu(rid)
    if use_pc:
    ...
    filename = \
        file_path + datetime.datetime.now().strftime('%Y-%m-%d_%H-%M_') + 'DouYu_' \
        + str(rid) + '.mp4'
    ffdm = ffdownloader.FFDownloaderWrapper(url, filename)
    logging.info('File path: ' + filename)
...
def main(use_pc: bool = True):
    ...
    try:
        resolve_and_download(config.rid, config.savePath, use_pc)
